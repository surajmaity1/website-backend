import {
  ...
  DISCORD_USER_CREATE_OOO_REQUEST,
...
import { CreateOooRequestBody, OooRequestCreateRequest, OooStatusRequest } from "../types/oooRequest";
import { UpdateRequest } from "../types/requests";

export const createOooRequestController = async (req: OooRequestCreateRequest, res: CustomResponse) => {
  const requestBody = req.body as CreateOooRequestBody;
  ...
  if (!isUserPartOfDiscord) {
    return res.boom.unauthorized(DISCORD_USER_CREATE_OOO_REQUEST);
  }
  ...
  try {
    ...
    if (!userStatus || userStatus.currentStatus.state === userState.OOO) {
